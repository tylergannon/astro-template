...
- Known rsvelte 0.5.2 limitation: in `.svelte.js`/`.svelte.ts` rune modules, a non-rune class field with a multi-line initializer after a rune-backed field can fail with `[PARSE_ERROR] Unexpected token`. Keep that initializer on one line. `mode-watcher` carries this narrow source patch, and `vp run check:shadcn` distinguishes that bug from working one-line and no-rune controls. An [upstream rsvelte](https://github.com/baseballyama/rsvelte/issues) fix prints a non-blocking notice to remove the workaround.
