...
- This starter is intentionally dark-only: the shadcn tokens, browser color scheme, hero, and Sonner wrapper all use one dark theme. Add an explicit theme switcher before introducing light or system mode.
- `src/lib/components/ui/sonner/sonner.svelte` intentionally differs from the registry: it hardcodes `theme="dark"` and omits `mode-watcher`. Preserve that change after `shadcn-svelte add sonner --overwrite`, or add a complete theme switcher and restore the dependency.
- Known rsvelte 0.5.2 limitation: in `.svelte.js`/`.svelte.ts` rune modules, a non-rune class field with a multi-line initializer after a rune-backed field can fail with `[PARSE_ERROR] Unexpected token`. Keep that initializer on one line. `vp run check:shadcn` distinguishes that bug from working one-line and no-rune controls; an [upstream rsvelte](https://github.com/baseballyama/rsvelte/issues) fix prints a non-blocking removal notice.
- Known Astro limitation: the installed `form` registry item composes `formsnap` with `sveltekit-superforms`; the latter's runtime requires SvelteKit's `$app/*` modules. Its source is retained for complete registry coverage, but `Form.Field` cannot be instantiated in this Astro template. Use the framework-neutral Field, Input, Label, and related components for Astro forms.
